...
select语句执行时，每次处理中选择case是比较随机的，
而且每次只挑其中一个分支case执行。
如果当下那个时间点没有一个 case 满足条件，就会走 default 分支（至多只能有一个 default 分支）。
...
select语句一般配合无限for循环进行，且在goroutine中的分线程执行。
